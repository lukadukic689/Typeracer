...
int fillArrayWithText(const char* path) { //returns if its possible to read a file (prevents crashes)
	FILE* fp;
	char letter;
	int i = 0;
	char* realPath = NULL;
...
float getWPM(const char* username) { //retursn wpm
...
void getLimitedString(char* string, int limit) {
	...
	if (size > limit - 1) {
		printf("\nIme mora sadržavati manje od 20 solva / brojeva\n");
		getLimitedString(string, limit);
	}
	else {
		return 0;
	}
