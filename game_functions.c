...
float cmdControl(const char* const text, char* unsolvedText, char* correctText, char* wrongText, int size, float oldWPM, char* username) { //returns time from end of the countdown till the end of the gamw
	...
	timer = clock();
		...
		for (int i = 0;; i++) {
			...
			letter = _getch();
			if (letter != '\b' && iPreview < 25 && !(*(text + i) == '\0')) {
			...
			else if ((IS_A_PRINTABLE_CHAR || letter == 32) && !(*(text + i) == '\0')) { //32 = space
...
void displayText(const char* unsolvedText,const char* correctText, const char* wrongText, const char* previewText, int incorrect, const char* timerText) {
	HANDLE hConsole = GetStdHandle(STD_OUTPUT_HANDLE);
	system("cls");
